...
import { History, ShoppingCart, Package, AlertTriangle, Calendar, Search, DollarSign, TrendingUp, Eye } from 'lucide-react';
import api from '../api';
...
export default function HistoryPage() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const result = await api.get(`/history/${activeTab}`);
            setData(result.data || []);
        } catch (e) {
                        ...
                        <table className="w-full">
                            <thead className="bg-card/50 border-b border-border/50">
                                <tr>
                                    ...
                                    {activeTab === 'sales' && (
                                        <>
                                            <th className="px-4 py-3 text-left text-sm font-medium text-muted-foreground">Folio</th>
                                    ...
                                    <tr key={index} className="hover:bg-card/50 transition-colors">
                                        ...
                                        {activeTab === 'sales' && (
                                            <>
                                                ...
                                                <td className="px-4 py-3 text-sm">{item.items_count || 0} items</td>
                                        ...
                                        {activeTab === 'purchases' && (
                                            <>
                                                <td className="px-4 py-3 text-sm">{item.supplier || 'N/A'}</td>
                                                <td className="px-4 py-3 text-sm text-right font-mono font-medium">
                                                    ${(item.total || 0).toFixed(2)}
                                                </td>
                                            </>
                                        ...
                                        {activeTab === 'losses' && (
                                            <>
                                                <td className="px-4 py-3 text-sm">{item.product_name || 'N/A'}</td>
                                                <td className="px-4 py-3 text-sm text-center">{item.quantity || 0}</td>
                                                <td className="px-4 py-3 text-sm text-muted-foreground">{item.reason || 'Sin motivo'}</td>
                                            </>
                                        )}
